...
from typing import Literal, Any, Generator, Dict, Iterator
...
class GraphStreamer:
    """Stream handler for SAP agent graph responses."""

    def __init__(self, graph: Any) -> None:
        """Initialize graph streamer."""
        self.graph = graph
...
test_agent = create_react_agent(
    model=model,
    tools=[get_table_schema, get_source_code, get_service_metadata],
    prompt="""
        You are an intelligent SAP integration agent with access to tools that interact with SAP APIs.
        ...
        - Before invoking any tool that calls an SAP API, always use the `get_service_metadata` tool first to fetch the API metadata.  
        - Use this metadata to understand the API structure, parameters, and HTTP method.  
...
response_generator = GraphStreamer(test_agent)

# Example usage
if __name__ == "__main__":

    # input = "hi, tell me about get_table_schema tool which you have and which API endpoints are available?"
    input = "Get source code for ZCL_JIRA_ISSUES from D2A system"
